...
function StatsSection({ data, sermons }: StatsProps) {
  ...
  const totalSize =
    sermons?.reduce((sum, s) => sum + Number(s.fileSize || 0), 0) || 0;
  const totalPlays =
    sermons?.reduce((sum, s) => sum + (s.playCount || 0), 0) || 0;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      {stats.map((stat, index) => (
        <div
          key={index}
          className="group relative overflow-hidden rounded-2xl bg-white border border-gray-100 hover:shadow-xl transition-all duration-300 hover:scale-[1.02]"
        >
          <div className="p-6">
    ...
    <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-8">
      <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
        ...
        <div className="flex gap-3">
          <Button
            variant="outline"
            className="rounded-xl h-9 px-4 text-sm border-gray-200 text-gray-700 hover:bg-gray-100 hover:text-teal-600 transition-colors"
          >
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="sm"
                  className="ml-2 hover:bg-gray-100 rounded-lg"
                >
        ...
        <div className="px-6 pb-6">
          ...
          {sermon.category && (
            <div className="mb-4">
              <Badge
                ...
                style={{
                  backgroundColor: sermon.category.color
                    ? sermon.category.color + "20"
                    : undefined,
                  borderColor: sermon.category.color || undefined,
                }}
              >
                {typeof sermon.category === "object"
          ...
          <div className="flex items-center justify-between text-sm text-gray-500 mb-4">
            ...
            <div className="flex items-center gap-1">
              ...
              <span>
                {sermon.fileSize
                  ? formatFileSize(Number(sermon.fileSize))
                  : "Unknown"}
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <Badge
              variant={sermon.isActive ? "default" : "secondary"}
              className={sermon.isActive ? "bg-green-100 text-green-700" : ""}
            >
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              audio sermon and remove the file from storage.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
...
export default function AudioSermonsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [
      "audio-sermons",
      { page: currentPage, search: searchTerm, limit: 20 },
    ],
    queryFn: () =>
      audioSermonsApi.getAll({
        page: currentPage,
        search: searchTerm,
        limit: 20,
        sortBy: "createdAt",
              ...
              <div className="flex flex-col items-center space-y-4">
                ...
                <div>
                  <h3 className="text-xl font-semibold text-gray-900 mb-2">
                    No audio sermons found
                  </h3>
                  <p className="text-gray-500 mb-6">
                    Upload your first audio sermon to get started
                  </p>
                  <Button
                    asChild
                    size="lg"
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center pt-8 gap-4">
              <p className="text-sm text-gray-600">
                ...
                {Math.min(currentPage * 20, data?.total || 0)} of{" "}
                {data?.total || 0} audio sermons
              </p>
